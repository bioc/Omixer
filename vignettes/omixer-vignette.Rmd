...
The number of iterations performed by the `omixerRand` function can be altered 
using the `iterNum` option. By default, this is set to 1,000 but in complex 
...
the run time however, and it may be preferable to run `omixerRand` with a 
lower `iterNum` initially, and inspect the resulting correlations in order 
...
After loading `.Random.seed`, you can run `omixerSpecific` to regenerate the 
optimal layout.
...
`group` option, and this is demonstrated in the extended example. If the text 
labels of the sample sheets are too large, then these can additionally be 
changed using the `group.text.size` and `sample.text.size` options. 
...
* Age and plate (r = -0.117, p = 0.004)
* BMI and chip (r = -0.094, p = 0.02)
* Date and plate (r = 0.127, p = 0.002)
...
layout, and the largest correlation coefficient returned is -0.052, which is 
